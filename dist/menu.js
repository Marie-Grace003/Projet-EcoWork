...
class Menu {
    ...
    async afficherMenuPrincipal() {
        ...
        while (continuer) {
            ...
            console.log("\n  0. Quitter");
            console.log("=".repeat(60));
            const choix = await (0, index_js_1.askNumber)("\n👉 Votre choix : ", 0, 13);
            switch (choix) {
                case 1:
    ...
    async ajouterLivre() {
        ...
        const titre = await (0, index_js_1.ask)("Titre : ");
        const auteur = await (0, index_js_1.ask)("Auteur : ");
        const isbn = await (0, index_js_1.ask)("ISBN : ");
        const anneePublication = await (0, index_js_1.askNumber)("Année de publication : ", 1000, 2100);
        console.log("\nCatégories disponibles : Roman, Sciences, Histoire, Autre");
        const categorie = await (0, index_js_1.ask)("Catégorie : ");
        const nombrePages = await (0, index_js_1.askNumber)("Nombre de pages : ", 1);
    ...
    async modifierLivre() {
        ...
        console.log("-".repeat(40));
        this.afficherLivres();
        const livreId = await (0, index_js_1.ask)("\nID du livre à modifier : ");
        ...
        const titre = await (0, index_js_1.ask)(`Titre [${livre.titre}] : `);
        const auteur = await (0, index_js_1.ask)(`Auteur [${livre.auteur}] : `);
        const isbn = await (0, index_js_1.ask)(`ISBN [${livre.isbn}] : `);
        const anneeStr = await (0, index_js_1.ask)(`Année [${livre.anneePublication}] : `);
        const categorie = await (0, index_js_1.ask)(`Catégorie [${livre.categorie}] : `);
        const pagesStr = await (0, index_js_1.ask)(`Pages [${livre.nombrePages}] : `);
        const modifications = {};
        if (titre)
        ...
        if (isbn)
            modifications.isbn = isbn;
        if (anneeStr)
            modifications.anneePublication = parseInt(anneeStr);
        if (categorie)
            modifications.categorie = categorie;
        if (pagesStr)
            modifications.nombrePages = parseInt(pagesStr);
    ...
    async supprimerLivre() {
        console.log("\n🗑️ SUPPRIMER UN LIVRE");
        console.log("-".repeat(40));
        this.afficherLivres();
        const livreId = await (0, index_js_1.ask)("\nID du livre à supprimer : ");
        const confirme = await (0, index_js_1.askConfirm)("⚠️  Êtes-vous sûr de vouloir supprimer ce livre ?");
        if (!confirme) {
    ...
    afficherLivres() {
        ...
        livres.forEach((livre) => {
            console.log(livre.id.padEnd(12) +
    ...
    async rechercherLivre() {
        ...
        const auteur = await (0, index_js_1.ask)("Auteur : ");
        const isbn = await (0, index_js_1.ask)("ISBN : ");
        const dispStr = await (0, index_js_1.ask)("Disponible uniquement ? (o/n) : ");
        const critere = {};
        if (titre)
            critere.titre = titre;
        if (auteur)
            critere.auteur = auteur;
        if (isbn)
            critere.isbn = isbn;
        if (dispStr.toLowerCase() === "o")
            critere.disponible = true;
    ...
    // ========================================
    async inscrireMembre() {
        console.log("\n👤 INSCRIRE UN NOUVEAU MEMBRE");
        console.log("-".repeat(40));
        const nom = await (0, index_js_1.ask)("Nom : ");
        const prenom = await (0, index_js_1.ask)("Prénom : ");
        const email = await (0, index_js_1.ask)("Email : ");
    ...
    async modifierMembre() {
        ...
        console.log("-".repeat(40));
        this.afficherMembres();
        const membreId = await (0, index_js_1.ask)("\nID du membre à modifier : ");
        ...
        console.log("(Laissez vide pour conserver la valeur actuelle)");
        const nom = await (0, index_js_1.ask)(`Nom [${membre.nom}] : `);
        const prenom = await (0, index_js_1.ask)(`Prénom [${membre.prenom}] : `);
        const email = await (0, index_js_1.ask)(`Email [${membre.email}] : `);
        const modifications = {};
        if (nom)
    ...
    afficherMembres() {
        ...
        membres.forEach((membre) => {
            console.log(membre.id.padEnd(12) +
    ...
    async retournerLivre() {
        ...
        empruntsActifs.forEach((emprunt) => {
            console.log(`  ${emprunt.id} - ${emprunt.livre.titre} (${emprunt.membre.prenom} ${emprunt.membre.nom})`);
        });
        const empruntId = await (0, index_js_1.ask)("\nID de l'emprunt : ");
    ...
    afficherEmpruntsActifs() {
        ...
        emprunts.forEach((emprunt) => {
            ...
            console.log(emprunt.id.padEnd(12) +
